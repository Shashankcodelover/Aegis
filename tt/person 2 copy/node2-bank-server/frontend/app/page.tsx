...
export default function BankPage() {
  ...
  useEffect(() => {
    const socket = io(BANK_HOST, { transports: ['polling'] });
  ...
  // ── AEGIS Toggle ──
  const toggleAegis = useCallback(() => {
    const newScenario = activeScenario === 'SCENARIO_3' ? 'SCENARIO_1' : 'SCENARIO_3';
  ...
  const doTransfer = useCallback(
    async (
      ...
      // NOTE: Do NOT emit set_scenario here — scenario is controlled only by the AEGIS toggle button.
      // Each column's buttons always send to the bank using the CURRENT active scenario on the backend.
        ...
        <header className="bg-white border-b border-stone-200 shadow-sm sticky top-0 z-40">
          <div className="flex items-center justify-between px-8 py-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={toggleAegis}
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold border-2 transition-all duration-200 ${
                  activeScenario === 'SCENARIO_3'
                    ? 'bg-teal-600 border-teal-500 text-white shadow-[0_0_16px_rgba(20,184,166,0.5)]'
                    : 'bg-stone-100 border-stone-300 text-stone-600 hover:border-teal-400 hover:text-teal-700'
                }`}
              >
                <ShieldCheck size={14} />
                {activeScenario === 'SCENARIO_3' ? '🛡 AEGIS ACTIVE' : 'ACTIVATE AEGIS'}
              </button>
              <div
